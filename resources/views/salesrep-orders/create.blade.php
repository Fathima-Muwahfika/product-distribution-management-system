...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route('salesrep.orders.store') }}" id="orderForm">
            @csrf
                    ...
                    <table class="table table-bordered" id="productTable">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th style="width:13%">Price (MRP)</th>
                        ...
                        <tbody id="productRows">
                            <tr class="product-row">
                                ...
                                <td><span class="subtotal-display fw-bold" style="font-size:13px;color:#0f2744;">LKR 0.00</span></td>
            ...
            <div class="d-flex justify-content-between align-items-center mt-3 p-3" style="background:#f8fafc;border-radius:12px;border:1px solid #e2e8f0;">
                ...
                <div style="font-size:20px;font-weight:700;color:#0f2744;">
                    LKR <span id="totalAmount">0.00</span>
                </div>
            </div>

            <div class="mt-4 d-flex gap-2">
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-save me-1"></i> Submit Order
                </button>
                <a href="{{ route('salesrep.orders') }}" class="btn btn-secondary">Cancel</a>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    function updateProductInfo(select) {
        ...
        if (productId && productsData[productId]) {
            ...
            row.querySelector('.stock-display').innerHTML =
                '<span class="badge ' + (product.stock_qty > 10 ? 'bg-success' : 'bg-danger') + '">'
                + product.stock_qty + '</span>';
            row.querySelector('.price-display').textContent =
                'LKR ' + parseFloat(product.mrp).toFixed(2);
    ...
    function updateRowSubtotal(row) {
        ...
        if (productId && productsData[productId]) {
            const subtotal = productsData[productId].mrp * qty;
            subtotalEl.textContent = 'LKR ' + subtotal.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',');
        } else {
    ...
    document.getElementById('addRow').addEventListener('click', function () {
        ...
        newRow.querySelector('.subtotal-display').textContent = 'LKR 0.00';
